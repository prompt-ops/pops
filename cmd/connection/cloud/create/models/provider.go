...
func (m providerSelectionModel) Quitting() bool {
	return m.quitting
}

func (m providerSelectionModel) Choice() string {
	return m.choice
}

func NewProviderSelectionModel() providerSelectionModel {
	cloudProviders := []string{"azure"}
	items := make([]list.Item, len(cloudProviders))
	...
	l.Title = "Select a cloud provider"
